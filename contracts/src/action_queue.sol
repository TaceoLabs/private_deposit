...
library QueryMapLib {
    function insert(QueryMap storage self, uint256 key, ActionQuery memory value) public returns (bool replaced) {
    ...
    function iterateStart(QueryMap storage self) public view returns (Iterator) {
        return iteratorSkipDeleted(self, 0);
    }

    function iterateValid(QueryMap storage self, Iterator iterator) public view returns (bool) {
        return Iterator.unwrap(iterator) < self.keys.length;
    }

    function iterateNext(QueryMap storage self, Iterator iterator) public view returns (Iterator) {
        return iteratorSkipDeleted(self, Iterator.unwrap(iterator) + 1);
    }

    function iterateGet(QueryMap storage self, Iterator iterator)
        public
        view
        returns (uint256 key, ActionQuery memory value)
    {
    ...
    function iteratorSkipDeleted(QueryMap storage self, uint256 keyIndex) private view returns (Iterator) {
