...
use co_noir::Bn254;
use co_noir_to_r1cs::{
    circom::{proof_schema::CircomProofSchema, solidity_verifier},
    noir::r1cs,
};
use itertools::izip;
use mpc_core::{
...
const PATH: &str = "/../contracts/src/groth16_verifier_compressed.sol";
...
fn test_process_mpc<R: Rng + CryptoRng>(
        ...
        for (nets, map, transaction) in izip!(
            ...
            let handle = scope.spawn(move || {
                ...
                let (
                    ...
                    _proof_time,
                ) = map.process_queue_with_cocircom_proof_compressed(
                    transaction,
                    circuit,
                    proof_schema,
                ...
                Result::<_, eyre::Report>::Ok((proof, public_inputs, new_balance_commitments))
            });
            handles.push(handle);
        }

        let (proof0, public_inputs0, new_balance_commitments0) = handles
            .remove(0)
            .join()
            .map_err(|_| eyre::eyre!("A thread panicked while processing a transaction"))??;
        for handle in handles {
            let (proof, public_inputs, new_balance_commitments) = handle
                .join()
                .map_err(|_| eyre::eyre!("A thread panicked while processing a transaction"))??;
            assert_eq!(proof, proof0);
            assert_eq!(public_inputs, public_inputs0);
            assert_eq!(new_balance_commitments, new_balance_commitments0);
        }
        Result::<_, eyre::Report>::Ok((proof0, public_inputs0, new_balance_commitments0))
    });
    let (proof, public_inputs, new_balance_commitments) = result?;

    // Verifiy the results
    if !r1cs::verify(&proof_schema.pk.vk, &proof, &public_inputs)? {
    ...
    println!("uint256 alpha = {};", public_inputs[2]);
    println!("uint256 beta = {};", public_inputs[0]);
    println!("uint256 gamma = {};", public_inputs[1]);

    println!(
        "uint256 amount_commitment = {};",
        new_balance_commitments[0]
    );
    println!(
        "uint256 alice_deposit_commitment = {};",
        new_balance_commitments[1]
    );
    println!(
        "uint256 alice_transfer_commitment = {};",
        new_balance_commitments[2]
    );
    println!(
        "uint256 bob_transfer_commitment = {};",
        new_balance_commitments[3]
    );
    println!(
        "uint256 bob_withdraw_commitment = {};",
        new_balance_commitments[4]
    );
...
fn main() -> eyre::Result<ExitCode> {
    ...
    let pa = TestConfig::get_transaction_batched_circom_compressed()?;
    let proof_schema = TestConfig::get_transaction_batched_proof_schema_compressed(&mut rng)?;
    let mut result = Vec::new();
    solidity_verifier::export_solidity_verifier(&proof_schema.pk.vk, &mut result)?;
    ...
    test_process_mpc(&pa, &proof_schema, keys, &mut rng)?;
