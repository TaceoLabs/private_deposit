...
fn create_action_queues<R: Rng + CryptoRng>(
    ...
    let mut action_queue_0 = Vec::with_capacity(NUM_BATCHED_TRANSACTIONS);
...
fn test_process_mpc<R: Rng + CryptoRng>(
    ...
    // Get action queues
    let action_queues = create_action_queues(alice, bob, amount, amount_blinding, rng);
...
fn main() -> eyre::Result<ExitCode> {
    ...
    test_process_mpc(&proof_schema, &cs, &pk, &mut rng)?;
