...
where
    K: std::hash::Hash + Eq,
{
    fn poseidon_helper<const I: usize, const I2: usize, N: Network>(
        ...
        let (_, traces) = hasher.hash_rep3_generate_noir_trace_many::<N, I, I2>(
            input,
            &mut hasher_precomp,
            net,
        )?;
    ...
    pub fn deposit_with_ultrahonk_proof<N: Network>(
        ...
        let traces = Self::poseidon_helper::<NUM_COMMITMENS, _, _>(
            [
                amount,
                amount_blinding,
                old_amount,
...
mod tests {
    ...
    fn deposit_ultrahonk_test() {
        ...
        for _ in 0..TestConfig::TEST_RUNS {
            ...
            let amount = if let Some(v) = read_plain {
                amount + v.amount
            } else {
                amount
            };
            plain_map.insert(key, DepositValuePlain::new(amount, F::zero()));
