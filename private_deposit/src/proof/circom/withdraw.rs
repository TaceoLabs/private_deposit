    ...
    K: std::hash::Hash + Eq,
{
    ...
    pub fn withdraw_with_cocircom_witext<N: Network>(
        &mut self,
        key: K,
        amount: Rep3PrimeFieldShare<F>,
        ...
        let inputs = Self::get_withdraw_circom_input(old, amount, amount_blinding, new.blinding);
    ...
    #[expect(clippy::too_many_arguments)]
    pub fn withdraw_with_cocircom_proof<N: Network>(
        &mut self,
        key: K,
        amount: Rep3PrimeFieldShare<F>,
        amount_blinding: Rep3PrimeFieldShare<F>, // For the commitment to the amount
        ...
        let (new, witness) = self.withdraw_with_cocircom_witext(
            key,
            amount,
            amount_blinding,
            circuit,
...
mod tests {
    ...
    #[test]
    fn withdraw_cocircom_test() {
        // TestConfig::install_tracing();

        // Init Groth16
        // Read circom file
        let pa = TestConfig::get_withdraw_circom().unwrap();
        let pa = Arc::new(pa);

        // Get the R1CS proof schema
        let mut rng = rand::thread_rng();
        let proof_schema = TestConfig::get_withdraw_proof_schema(&mut rng).unwrap();
